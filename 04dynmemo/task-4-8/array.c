...
int array_create_struct(struct array_t **a, int N) {
    ...
    *a = malloc(sizeof(struct array_t));
    if (*a == NULL) return ARRAY_CREATE_ALLOC_FAIL;

    if (array_create(*a, N) == ARRAY_CREATE_ALLOC_FAIL) {
        free(*a);
        return ARRAY_CREATE_ALLOC_FAIL;
    }
    return ARRAY_CREATE_OK;
}
...
int array_remove_item(struct array_t *a, int value) {
    if (a == NULL) return -1;
    if (a->ptr == NULL || a->capacity <= 0 || a->size < 0 || a->size > a->capacity) return -1;
